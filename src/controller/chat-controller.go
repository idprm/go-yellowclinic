...
func ChatUser(c *fiber.Ctx) error {
	...
	var chat model.Chat
	database.Datasource.DB().Joins("Order", &model.Order{UserID: user.ID, Voucher: req.Voucher}).Where(&model.Chat{UserID: user.ID, IsLeave: false}).Preload("User").Preload("Doctor").Preload("Order").First(&chat)
