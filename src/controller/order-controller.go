...
func OrderChat(c *fiber.Ctx) error {
	...
	if resultOrder.RowsAffected == 0 {
		...
		var chat model.Chat
		resultCount := database.Datasource.DB().Joins("Order", database.Datasource.DB().Where(&model.Order{UserID: user.ID, Voucher: user.LatestVoucher})).Where(&model.Chat{IsLeave: false}).First(&chat)
